...
import { startChat, sendMessage, endChat, fetchSessionHistory, onAddCommonFormApi, onGetCommonApi, onAddCommonJsonApi } from '../../api/api'
...
const AdminMessageList = ({ route, navigation }) => {
  ...
  const onGetChatHistory = async () => {
    try {
      ...
      const response = await onAddCommonJsonApi('admin-chat-history?page=1&per_page=100', raw);
      if (response.data.status) {
        console.log('Get onGetChatHistory', response.data.users);
        ...
        setReceiverData(response.data.users.sender.user_type != 'astrologer' ? response.data.users.sender : response.data.users.receiver);
        setIsLoading(false);
        setTimeout(() => {
  ...
  const sendUserMessage = async () => {
    ...
    try {
      ...
      formdata.append("sender_id", astrologer.id);
      ...
      console.log('onSendMessage Error:', error.response);
      setMessageLoading(false);
    }
  };
