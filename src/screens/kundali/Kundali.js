...
import { generateKundali } from '../../api/api'
...
export default function Kundali({ navigation, route }) {
  ...
  const allFilled = name && dob && time && place && gender && language
  const wallet = costs?.wallet
  const hasInsufficientBalance = wallet !== null && costs?.kundali_cost && wallet < costs.kundali_cost
  ...
  const onGenerate = async () => {
    ...
    try {
      ...
      console.log('Get Request Data:::', formData);
      const response = await generateKundali(formData)
      
      // Check if response status is false
      if (response && response.status === false) {
        Alert.alert('Error', response.message || 'Failed to generate kundali')
        return
      }
      ...
      navigation.navigate('ViewKundali', { 
        kundaliData: response.text || response.data || response,
        personalDetails: formData,
      })
    } catch (error) {
      console.error('Error generating kundali:', error)
      let errorMessage = 'Failed to generate kundali. Please try again.'
      
      // Check if error response has status false and message
  ...
  return (
    <SafeAreaView style={[styles.safe, {paddingBottom: insets.bottom, paddingTop: insets.top}]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Kundali Generation</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      >
        ...
        >
          <View style={styles.heroSection}>
            ...
            <Text style={styles.pageTitle}>Create your Kundali</Text>
            <Text style={styles.lead}>
              Enter birth details to generate your detailed birth chart and planetary positions.
            </Text>
          </View>
          ...
          {costs?.kundali_cost && (
            <View style={styles.costBanner}>
              ...
              <Text style={styles.costValue}>
                {costs.currency || '₹'}{costs.kundali_cost}
              </Text>
            </View>
          )}

          {/* Insufficient Balance Warning */}
          {wallet !== null && costs?.kundali_cost && wallet < costs.kundali_cost && (
              ...
              <View style={styles.warningContent}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.warningText}>
                    Your wallet balance ({costs.currency || '₹'}{wallet}) is less than required amount ({costs.currency || '₹'}{costs.kundali_cost})
                  </Text>
                </View>
              </View>
              <TouchableOpacity 
        ...
        {!hasInsufficientBalance && (
          <View style={styles.footer}>
            <TouchableOpacity 
              style={[styles.continueBtn, (!allFilled || loading) && styles.continueDisabled]} 
              onPress={onGenerate}
            ...
            >
              {loading ? (
                ...
                <>
                  <Text style={styles.continueText}>Generate Kundali</Text>
